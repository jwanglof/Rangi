...
public class BlockFilter{
    ...
    private int blockSizeH = 2;
    private int blockSizeW = 2;
    ...
    public BlockFilter( int blockSizeH, int blockSizeW ) {
    ...
    public void setBlockSize(int blockSizeH, int blockSizeW) {
        this.blockSizeH = blockSizeH;
        this.blockSizeW = blockSizeW;
    }
    ...
    public int[] filter2( int[] src ,int W, int H ) {
        int width = W;
        int height = H;

        int[] dst = new int[width * height];
        int[] pixels = new int[blockSizeH * blockSizeW];
        for ( int y = 0; y < height; y += blockSizeH ) {
            for ( int x = 0; x < width; x += blockSizeW ) {
                int w = Math.min( blockSizeW, width-x );
                int h = Math.min( blockSizeH, height-y );
                int t = w*h;
                PixelUtils.getRGB( src, x, y, w, h, width , pixels );
                int r = 0, g = 0, b = 0;
                int argb;
